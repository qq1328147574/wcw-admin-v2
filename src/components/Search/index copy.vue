<template>
  <div class="search" v-if="formColumnData.length > 0">
    <el-form 
      ref="searchForm" 
      class="searchForm"
      :model="searchData" 
    ...
    >
      <el-row>
        <el-col :xs="12" :sm="12" :md="8" :lg="6" :xl="5" v-for="(item,index) in formColumnData" :key="index">
          <el-form-item class="search-item" :label="item.label"
              label-position="top">
            <!-- slot -->
            <slot :name="item.slot" v-if="item.slot" :default="searchData"></slot>
            <!-- checkbox -->
            <el-checkbox v-else-if="item.type === 'checkbox'" v-model="searchData[item.key]"></el-checkbox>
            <!-- select -->
            <el-select v-else-if="item.type === 'select'" v-model="searchData[item.key]" :placeholder="item.placeholder || ''">
              <el-option v-for="_item in item.options" :key="_item.value" :label="_item.label" :value="_item.value"> </el-option>
            </el-select>
            <!-- date-picker time1 -->
            <el-date-picker
            ...
            <!-- input -->
            <el-input :type="item.type" v-model="searchData[item.key]" :placeholder="item.placeholder || ''" v-else></el-input>
          </el-form-item>
        </el-col>
        <el-col :xs="12" :sm="12" :md="8" :lg="6" :xl="4">
          <el-form-item>
            <el-button type="primary" @click="handleSearchClick" icon="el-icon-search" size="small">{{ $t('Common.搜索') }}</el-button>
            <el-button type="default" @click="handleResetClick" icon="el-icon-refresh-left" size="small">{{ $t('Common.重置') }}</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
