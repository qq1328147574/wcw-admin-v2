 ...
 **/
  ...
  */
 export const regMakeUpPassMin = /^(([a-z]+[0-9]+)|([0-9]+[a-z]+))[a-z0-9]*$/i;
 export const regMakeUpPass = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z@!+=]{7,17}$/;
 // 2- 包含数字和字符
 export const regZmNum = /^[0-9\W!@#$%^&*?]{8,16}$/;
 
 // 验证数字，大写字母，小写字母，特殊字符四选三组成的密码强度，且长度在8到30个数之间
 export const AzPattern = /^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z\W_!@#$%^&*`~()-+=]+$)(?![a-z0-9]+$)(?![a-z\W_!@#$%^&*`~()-+=]+$)(?![0-9\W_!@#$%^&*`~()-+=]+$)[a-zA-Z0-9\W_!@#$%^&*`~()-+=]{8,16}$/;
